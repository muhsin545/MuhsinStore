...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
     ...
     <div id= "rating"> <p class="rounded rounded-pill"><span>
     <i class="star fas fa-star"></i>
     <i class="star fas fa-star"></i>
     <i class="star fas fa-star"></i>
     <i class="star fas fa-star"></i>
     <i id = "uniqueStar" class="fas fa-star"></i></span>
     <span> ${product.rating.rate}</span></p>
     <p id="userFeedBack"><span><i class="fas fa-users"></i></span><span> ${product.rating.count}</span></p></div>
      <h4>Price: $ ${product.price}</h4>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-outline-primary me-3 mt-1">add to cart</button>
      <button id="details-btn" class="btn btn-outline-danger ms-3 mt-1">Details</button></div>
